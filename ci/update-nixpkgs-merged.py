...
def nixpkgs_repository(directory: str, origin: str, branches: list[str]) -> Repo:
    ...
    for name, url in dict(origin=origin).items():
        ...
        getattr(repo.remotes, name).fetch(refspec=" ".join(branches), filter="blob:none")

    return repo


def rebase_nixpkgs(gh: Github, nixpkgs_repo: Repo, target_branch: str, integration_branch: str) -> bool:
    ...
    gh.get_repo(NIXPKGS_REPO).get_git_ref(f"heads/{integration_branch}").delete()
    return True
...
def cleanup_old_prs_and_branches(gh: Github, merged_integration_branch: str):
    ...
    for repo in [fc_nixos_repo, nixpkgs_repo]:
        for branch in repo.get_branches():
            ...
            if datetime.date.fromisoformat(branch_datestr) < merged_integration_branch_date:
...
def main():
    ...
    argparser.add_argument("--merged-pr-id", help="merged fc-nixos PR ID", required=True)
    ...
    nixpkgs_repo = nixpkgs_repository(args.nixpkgs_dir, args.nixpkgs_origin_url, [integration_branch, nixpkgs_target_branch])
    if rebase_nixpkgs(nixpkgs_repo, nixpkgs_target_branch, integration_branch, ):
        fc_nixos_pr.create_issue_comment(f"Rebased nixpkgs `{nixpkgs_target_branch}` branch successfully.")
